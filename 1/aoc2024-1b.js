...
const input = fs.readFile('1/aoc2024-1_input.txt', (err, input) => {
    if (err) throw err
    data = input.toString();
    data = data.split(/\n/) // split by lines
    ...
    for (const num in c1) {
        let filteredC2 = c2.filter(value => value == c1[num])
        // console.log(c1[num] * filteredC2.length)
        similarityScore += c1[num] * filteredC2.length
    }
